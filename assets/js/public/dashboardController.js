    ...
    $http.post('/search', {keyword: $scope.keyword})
      .then(function onSuccess (res) {
        ...
        function dashboard(id, fData){
          ...
          function pieChart(pD){
            ...
            function mouseout(d){
              ...
              hG.update(fData.map(function(v){
                return [v.State,v.freq.high];}), barColor);
            }
          ...
          var sF = fData.map(function(d){return [d.State,d.freq.high];});
          var hG = histoGram(sF), // create the histogram.
            pC = pieChart(tF), // create the pie-chart.
            leg= legend(tF);  // create the legend.
        }

        var data = $scope.response.data;
        ...
        for (var i=0; i<data.length; i++) {
          ...
          var low = parseInt(data[i].temperatureMin, 10);
          var high = parseInt(data[i].temperatureMax, 10);
          var mid = parseInt((low + high) / 2, 10);
          refineData.push({State: date, freq:{low: low, mid:mid, high: high}})
        }

        dashboard('#dashboard',refineData);
